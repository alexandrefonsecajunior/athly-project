...
const GetAllActivitiesInputSchema = z.object({
    ...
    maxActivities: z.number().int().positive().optional().default(200).describe("Maximum activities to return after filtering (default: 200)"),
    maxApiCalls: z.number().int().positive().optional().default(5).describe("Max API calls to avoid rate limit (Strava: 100/15min). Default 5 = up to ~1000 activities."),
