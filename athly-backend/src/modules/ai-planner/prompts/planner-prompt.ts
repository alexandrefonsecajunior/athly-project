...
function classifyAthlete(avgPace: string): string {
  ...
  if (totalSec > 345) return 'Beginner (slower than 5:45/km) — prioritize volume and consistency';
  if (totalSec > 312) return 'Progressing (5:13–5:44/km) — increase interval intensity';
  return 'Goal within reach (≤ 5:12/km) — refine race pace and pacing strategy';
}

/**
 * Prompt for athletes with no Strava running history.
 * Asks Gemini to generate 5 assessment workouts spread across the week.
 */
